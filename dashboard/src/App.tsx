...
const saoPauloDateFormatter = new Intl.DateTimeFormat('pt-BR', { timeZone: 'America/Sao_Paulo' })
const saoPauloTimeFormatter = new Intl.DateTimeFormat('pt-BR', { timeZone: 'America/Sao_Paulo', hour: '2-digit', minute: '2-digit', second: '2-digit' })
const saoPauloChartTimeFormatter = new Intl.DateTimeFormat('pt-BR', { timeZone: 'America/Sao_Paulo', hour: '2-digit', minute: '2-digit' })
...
export function App() {
  ...
  const groupedAlarms = useMemo(() => {
    const groups = new Map<string, Alarm & { count: number }>()
    alarms.forEach((alarm) => {
      const normalizedTitle = alarm.title.replace(/\b\d+\s+minutos?\b/gi, '<tempo>')
      const key = [alarm.severity, normalizedTitle, alarm.detail].join('|')
      const existing = groups.get(key)
      if (existing) existing.count += 1
      else groups.set(key, { ...alarm, count: 1 })
    })
    return Array.from(groups.values())
  }, [alarms])
  const highestSeverity = alarms.some((alarm) => alarm.severity === 'critical') ? 'ALTA' : alarms.length ? 'MÉDIA' : 'NORMAL'
    ...
    try {
      ...
      if (historyData && historyData.length > 0) {
        const formatted = historyData.map((h: any) => ({
          t: saoPauloChartTimeFormatter.format(new Date(h.timestamp)),
          temp: h.temperature,
          setpoint: h.setpoint
        }))
      ...
      if (alertsData && alertsData.length > 0) {
        setAlarms(alertsData.map((a: any) => ({
          ...
          title: a.message,
          detail: a.device_id,
          time: saoPauloTimeFormatter.format(new Date(a.created_at))
        })))
      }
    } catch (err) {
  ...
  return <main className="iceberg-shell">
    <header className="iceberg-header"><div className="iceberg-brand"><div className="mountain-mark"><Snowflake size={30} /></div><div><div className="brand-kicker">ICEBERG</div><h1>EXECUTIVE</h1><p>MONITORAMENTO PREMIUM</p></div><div className="brand-divider" /><div className="room-name"><b>CÂMARA FRIA <em>01</em></b><span>MONITORAMENTO EM TEMPO REAL</span></div></div><div className="header-meta"><span>DATA <b>{saoPauloDateFormatter.format(time)}</b></span><span>HORA <b>{saoPauloTimeFormatter.format(time)}</b></span></div><div className="header-actions"><div className="connection"><span className="pulse" /> SISTEMA <b>{connected ? 'CONECTADO' : 'OFFLINE'}</b></div><button className="round-action"><Bell size={17} /><sup>{alarms.length}</sup></button><div className="profile">AH</div></div></header>
...
function Alarm({ level, title, detail, time, count = 1, critical = false }: { level: string; title: string; detail: string; time: string; count?: number; critical?: boolean }) { return <div className={`alarm-item ${critical ? 'critical' : 'warning'}`}><span>{level}</span><div><b>{title}</b><small>{detail}{count > 1 ? ` • ${count} ocorrências` : ''}</small></div><time>{time}</time><strong>ATIVO</strong></div> }
